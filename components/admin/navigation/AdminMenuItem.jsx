...
export default function AdminMenuItem({
  ...
  useEffect(
    function openActiveMenu() {
      const equal_route = paths.some((item) => item === router.pathname);
      if (equal_route) {
        setIsActive(true);
      }
    },
      ...
      <header className="admin-menu-item__header">
        ...
        {!url && (
          <div className={icon_class}>
            <Image src="/svg/chevron-left.svg" width={20} height={16} alt="" />
          </div>
        )}
      </header>
            ...
            {items.map((item, index) => {
              ...
              return (
                <li className={activeClass} key={index}>
                  <div className="admin-submenu__icon">{submenu_icon}</div>
                  <Link href={item.url}>
                    {index === 0 ? <a>Add item</a> : <a>View list</a>}
                  </Link>
                </li>
              );
            })}
